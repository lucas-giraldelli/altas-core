...
export async function renderCheck(ROOT, slug) {
  ...
  let browser;
  try { browser = await chromium.launch(); } catch (e) { server.close(); return { ok: true, brokenDiagrams: [], skipped: 'chromium indisponível' }; }
  try {
    const page = await browser.newPage();
    const errors = [];
    page.on('pageerror', (e) => errors.push(e.message));
    await page.goto(`http://localhost:${server.address().port}/${slug}/`, { waitUntil: 'networkidle', timeout: 30000 });
    await page.waitForFunction(() => !document.querySelector('pre.mermaid:not([data-processed])'), null, { timeout: 15000 }).catch(() => {});
    await page.waitForTimeout(300);
    const brokenDiagrams = await page.evaluate(() => [...document.querySelectorAll('pre.mermaid')]

...
export interface Request extends RecordModel {
  ...
  kind: '' | 'fs' | 'edit'; payload: FsOp | EditOp | null;
...
export const enqueueEdit = (payload: EditOp) => col().create<Request>({ kind: 'edit', payload, content: `${payload.heading}: ${payload.instruction}`, status: 'pending' });
